...
from income.entity.artifact_entity import DataIngestionArtifact,DataValidationArtifact,ModelTrainerArtifact, ModelEvaluationArtifact
from income.component.model_trainer import evaluate_classification_model
from income.util.util import read_yaml_file,load_numpy_array_data,load_object,write_yaml_file,load_data
...
class ModelEvaluation:
        ...
        try:
            ...
            if previous_best_model is not None:
                ...
                if HISTORY_KEY not in model_eval_content:
                    history = {HISTORY_KEY}.update(model_history)
                    eval_result.update(history)
                else:
                    model_eval_content[HISTORY_KEY].update(history)
    ...
    def initiate_model_evaluation(self):
        try:
            ...
            #target column
            logging.info(f"converting target column into numy array")
            ...
            if model is None:
                logging.info(f"Not found any existing model. hence accepting training model")
                model_evaluation_artifact = ModelEvaluationArtifact(is_model_accepted=True,
                                                                    evaluated_model_path=trained_model_fie_path)
            ...
            if metric_info_artifact is None:
                response = ModelEvaluationArtifact(is_model_accepted=False,evaluated_model_path=trained_model_fie_path)
                logging.info(response)
                return response
            
            if metric_info_artifact.index_number == 1:
                model_evaluation_artifact = ModelEvaluationArtifact(is_model_accepted=True, evaluated_model_path=trained_model_fie_path)
                self.update_evaluation_report(model_evaluation_artifact=model_evaluation_artifact)
                logging.info(f"Model accepted. Model eval artifact  [{model_evaluation_artifact}] created")

            else:
                logging.info(f"Trained model is no better than existing model hence not accepting trained model")
                model_evaluation_artifact = ModelEvaluationArtifact(is_model_accepted=False, evaluated_model_path=trained_model_fie_path)
